...
export async function renderVehiculos(container, cliente_id, onClose) {
  container.innerHTML = `<div style="background:#fff; padding:2em; border-radius:8px; max-width:500px; margin:auto;">
    <h3>Vehículos</h3>
    <button id="nuevo-vehiculo">+ Nuevo vehículo</button>
    <div id="vehiculos-lista"></div>
    <button id="cerrar-vehiculos" style="margin-top:1em;">Cerrar</button>
    <div id="vehiculo-form-modal" style="display:none;"></div>
  </div>`;
  ...
  async function cargarVehiculos() {
    ...
    listaDiv.innerHTML = data.map(v => `
      <div style="border:1px solid #ccc; margin:1em 0; padding:1em; border-radius:8px;">
        <b>${v.marca || ''} ${v.modelo || ''}</b> - ${v.patente || ''} (${v.año || ''})<br>
        <button data-id="${v.id}" class="editar-vehiculo">Editar</button>
        <button data-id="${v.id}" class="eliminar-vehiculo" style="color:red;">Eliminar</button>
      </div>
  ...
  async function mostrarFormVehiculo(id) {
    ...
    modal.innerHTML = `
      <div style="background:#fff; padding:2em; border-radius:8px; max-width:400px; margin:auto;">
        <h4>${id ? 'Editar' : 'Nuevo'} vehículo</h4>
        <form id="vehiculo-form">
          <input name="marca" placeholder="Marca" value="${vehiculo.marca || ''}" required><br>
          <input name="modelo" placeholder="Modelo" value="${vehiculo.modelo || ''}" required><br>
          <input name="patente" placeholder="Patente" value="${vehiculo.patente || ''}" required><br>
          <input name="año" placeholder="Año" type="number" value="${vehiculo.año || ''}" required><br>
          <button type="submit">Guardar</button>
          <button type="button" id="cancelar-vehiculo">Cancelar</button>
        </form>
        <div id="vehiculo-form-error" style="color:red;"></div>
      </div>
    `;
